...
if selected == 'Abstract':
	...
	st.markdown('The World Happiness Report is a landmark survey of the state of global happiness. Nowadays, the increase in global recognition of the report is gaining new momentum, as happiness indicators are being widely applied across governments, companies and organisations to inform their policy-making decisions. These measurements of how happy citizens perceive themselves to be are proven to have an effective use in the evaluation of the progress of countries and regions.')
	st.markdown('In this case study, we will closely examine the data collected from the report and assess the difference between the national average of happiness scores in different countries, analyse the growth of happiness predictors over time as well as interpret the potential factors that affected the results. In addition, a prediction model will be constructed as a reference to future happiness values.')
	st.markdown('')
	st.markdown('')
	st.markdown('')
	st.markdown('')
...
if selected == 'Background Information':
	...
	st.markdown('This main dataset consists of data collected in various different attributes of world happiness from 2005 to 2020. It contains 1948 rows and 11 collumns, including the country name, year and their allotted statistics. Notice that there are some null values from certain countries in certain years.')
...
if selected == 'Data Cleaning':
	...
	st.markdown('In this process, the two datasets will be combined into one complete table to conduct analysis on. In order to achieve this, incorrectly formatted data will be fixed and irrelavent data will be removed. Through this process, the data is ensured to be correct, consistent and usable. ')
...
happiness_revised.drop(['Positive affect','Negative affect'],axis = 1,inplace=True)'''
	...
	st.markdown('Notice that though the two datasets shares many columns that measures the same variables, the naming of the columns are different. So then some columns of the 2021 dataset is renamed to match the main dataset in order to successfully combine the two later.')
...
happiness_2021 = happiness_2021[list(happiness_revised.columns) + ['Regional indicator']]'''
	...
	annotated_text('Last but not least, a region coulmn is added so that analysis can be done in comparison of different regions. A', ("Regional indicator", '', "#F0F3F4  "),'column can be found in the original 2021 dataset. The data in that column is used to create a dictionary that assigns each country with their region. The new column', ("Region", '', "#F0F3F4  "),'is then made in the concatenated dataset using the dictionary to finally complete this process.')
...
if selected == 'Familiarizing with Data':
	...
	st.markdown('The following sunburst chart helps visualize the proportion of representation from each region in the dataset. Bear in mind that the total number of countries from different regions vary. This chart provides a sense of which countries are included, and how much they contributed to the data.')
	st.caption('Click on any sector to focus on its attributted region and zoom in to get a closer look on the representation of the countries within. Click again on the center core of the plot to zoom back out.')
	merged_table = pd.read_csv('world_happiness2.csv')
	...
	annotated_text('')
	annotated_text(("Life Ladder", '', "#F0F3F4  "),": the national average of responces to the self-evaluation of the status of the respondent's life on a “ladder” scale ranging from 0 to 10, where 0 means the worst possible life and 10 the best possible life.")
	st.caption('Drag the slider to assess your own "life ladder" value')
	ll = st.slider('Evaluate your current life status: 0 being worst possible life, 10 being best possible life', value = 5, min_value = 0, max_value = 10, step = 1)
	...
	st.markdown('')
	st.markdown('')
	st.markdown('Now that we have a more concrete understanding about the data, we can start conducting analysis.')
...
if selected == 'Data Analysis':
	...
	col1.markdown('This scatter plot visualizes the happiness values of countries sorted in the regions to which they belong to. Notice how the y axis scale changes according to the region selected, this difference shows the variation of happiness levels across different regions.')
	col1.markdown('')
	col1.markdown('')
	...
	foption = col1.selectbox(
     'Select the format of display: by regional avereage or indivisual countries',
     ('Region','Country name'))
	...
	st.markdown('Through this box and whiskers plot visualization, outliers that are out of the upper and lower fence ranges of their regions are represented as indivisual dots and are easily iddentified. Such as Afghanistan, the hover data displays that it has the life ladder of 2.375, whereas the lower fence of its allotted region -South Asia- is 3.131. Other than identifying outliers, this plot also helps us to clearly visualize and compare the average as well as the range of values of different regions. It is easily observed that North America and ANZ and Western Europe are concentrated at a relatively high ladder score, on the other hand, South Asia and Sub-Saharan Africa have the lowest span.')
	...
	col5.markdown('This scatter plot shows the relation between values from 3 axes: \'Life Ladder\', \'Log GDP per capita\' and \'Generosity\', with each dot representing one country and their happiness status in the year 2021. The colors are attributed according the the region to which the country belongs to, helping visualize and diffrenciate the distribution of happiness levels in various regions.')
	...
	st.markdown('Through these visualisations, it is easily observed that there are trends between happiness factors of the countries from the same region. The statistics show that the happiness values of the regions Western Europe and especially North America and ANZ have a more compact structure - in other words the range of varying values for those two regions are relatively small (as seen on box plot) and has a high average score compared with other regions that are represented in the data. For there to be a high happiness score, the region\'s economy is a key contributing factor. Looking closer, it is recognised that countries of Western Europe are rich in agricultural and industrial diversity, generally have more developed economies and obtain a high level of income per capita. Moreover, North America, Australia and New Zealand are all developed countries. So it is easily justifiable for those regions to obtain high happiness levels. On the other hand, the values of South Asia and South-Saharan Africa are concentrated at a comparatively low span. This might be because these two regions mainly consist of developing countries, and the latter has the world\'s lowest total GDP.')
	st.header('Predictors progression')
	st.subheader('Line chart - growth of happiness factors overtime')
	...
	col8.plotly_chart(px.line(check_data, y = optionn, color = 'Country name', x= 'year', height = 500))
	st.markdown('In this graph, you can choose to visualize the progression of any happiness factor in the data set. In the texual analysis, however, we will focus on three major happiness factors and their trends over the years. Other than observing the growth of happiness factors overtime, we will also investigate the reasons behind sudden changes to the trendlines, and associate them with political, environmental or economical shifts in that allotted time period that are potentially responsible for the shifts. ')
	st.markdown('###### Life Ladder')
	st.markdown('The Life Ladder is the average value of citizens\'self-evaluation of their current status of life. Through the line graph, we can see that the life ladder of each country have many drops and rises throughout the years in no particular global pattern. However, it still can be seen that the general trend is that the life ladder values are growing overtime.')
	st.markdown('###### Healthy life expectancy at birth')
	st.markdown('On the other hand, the healthy life expectancy at birth has a relatively stable gorwth and a much more obvious trend. We can clearly see that - using the visualization - the healthy life expectancy has been increasing fairly steadily over the years, with no diruption to the rising pattern except for the year 2021, during which all the countries seems to have taken a dip in their values. With the coronavirus pandemic stretching on for the second year along with the emergance of new variants, and as the restrictions continue to be imposed upon citizens, professionals claim that the year brought on a \"new low\" in people\'s mental health globally. Not only were people exposed the risk of contracting the deadly virus which can severely damage health and major organs permanently, the added depression over citizens also lowers the life expectancy on a global level. Studies find that poor mental health can lead to decrease in longevity." ')
	st.markdown('###### Log GDP per capita')
	st.markdown('Similarly to healthy life expectancy at birth, log GDP per capita has a steady trendline with a general upward growth. One disruption to the trend is the year 2020, in which the majority of countries had a decrease in their GDP value but recovered some growth in the following year. The cause of that is likely to be the COVID-19 recession (a global economy recession due to the pandemic breakout), which started in most countries during early Feburary 2020. The pandemic significanly slowed consumer activity. Other than that, the 2020 stock market crash was also a major factor that contributed to the recession in ecconomic growth. And precausions that the governments took against it - such as the lockdown, limit in numbers at public spaces, etc. - rapidly increased unemployment and product prices.   ')
	st.markdown('')
	st.markdown('')
	st.markdown('')
	...
	if button : 
		...
		st.caption('Click to return data prediction when variables are inputed')
...
if selected == 'Conclusion':
	...
	st.markdown('As the society progresses onwards with the constant development of technological advancements, the environment, privileges and infrustructures available for citizens and the overall standard of living is also gradually rising. Through the analysis of the data, we can see that most of the happiness factors has an increasing trend in proportion to time on a global level. Though the recent pandemic has caused some degree of decline in the originally rising pattern, it does not make a significant impact on the general trend. And in the foreseeable future, it is speculated that the drop in happiness score will recover. ')
